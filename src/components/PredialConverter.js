...
function PredialConverter() {
  ...
  const [conversionType, setConversionType] = useState("20to30");
  const [ubicacion, setUbicacion] = useState(null);

  const getUbicacion = (depto, mpio) => {
    const deptoNombre = DEPARTAMENTOS[depto] || "Departamento no encontrado";
    const mpioNombre = MUNICIPIOS[depto + mpio] || "Municipio no encontrado";
    return { deptoNombre, mpioNombre };
  };

  const convertir_numero_predial_a_30_digitos = (predial_20_digitos) => {
    if (predial_20_digitos.length !== 20) {
      throw new Error("El número predial debe tener exactamente 20 dígitos.");
    }

    const depto = predial_20_digitos.slice(0, 2);
    const mpio = predial_20_digitos.slice(2, 5);

    // Obtener ubicación
    setUbicacion(getUbicacion(depto, mpio));

    const zona = predial_20_digitos.slice(5, 7);
    const sector = predial_20_digitos.slice(7, 9);
    const vereda_manzana = predial_20_digitos.slice(9, 13);
    const terreno = predial_20_digitos.slice(13, 17);
    const condicion_predio = predial_20_digitos.slice(17, 20);
  ...
  const convertir_numero_predial_a_20_digitos = (predial_30_digitos) => {
    if (predial_30_digitos.length !== 30) {
      throw new Error("El número predial debe tener exactamente 30 dígitos.");
    }

    const depto = predial_30_digitos.slice(0, 2);
    const mpio = predial_30_digitos.slice(2, 5);

    // Obtener ubicación
    setUbicacion(getUbicacion(depto, mpio));

    const zona = predial_30_digitos.slice(5, 7);
    const sector = predial_30_digitos.slice(7, 9);
    const vereda_manzana = predial_30_digitos.slice(13, 17);
    const terreno = predial_30_digitos.slice(17, 21);
    const condicion_predio = predial_30_digitos[22];
  ...
  const handleConvert = () => {
    try {
      setError("");
      if (conversionType === "20to30") {
        if (inputNumber.length !== 20) {
          throw new Error(
            "El número predial debe tener exactamente 20 dígitos"
          );
        }
        const converted = convertir_numero_predial_a_30_digitos(inputNumber);
        setResult(converted);
      } else {
        if (inputNumber.length !== 30) {
          throw new Error(
            "El número predial debe tener exactamente 30 dígitos"
          );
        }
        const converted = convertir_numero_predial_a_20_digitos(inputNumber);
        setResult(converted);
      }
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white rounded-lg shadow p-6">
      <h2 className="text-2xl font-bold text-center mb-2">
        Conversor de Números Prediales
      </h2>
      <h3 className="text-lg text-gray-600 text-center mb-6">(Rural)</h3>
      ...
      <div className="space-y-4">
        <div>
          ...
          >
            <option value="20to30">20 dígitos a 30 dígitos</option>
            <option value="30to20">30 dígitos a 20 dígitos</option>
          </select>
        </div>
        ...
        <div>
          <input
            type="text"
            placeholder={`Ingrese número predial de ${
              conversionType === "20to30" ? "20" : "30"
            } dígitos`}
            value={inputNumber}
            onChange={handleInputChange}
            maxLength={conversionType === "20to30" ? 20 : 30}
            className="w-full p-2 border rounded-lg font-mono"
          />
          <p className="text-sm text-gray-500 mt-1">
            Dígitos ingresados: {inputNumber.length} /{" "}
            {conversionType === "20to30" ? "20" : "30"}
          </p>
        </div>
        ...
        <button
          ...
          disabled={
            inputNumber.length !== (conversionType === "20to30" ? 20 : 30)
          }
      ...
      <div className="mt-8 text-center text-gray-500 text-sm">
        Creado por Sebastian Martinez
      </div>
    </div>
  );
}

export default PredialConverter;
